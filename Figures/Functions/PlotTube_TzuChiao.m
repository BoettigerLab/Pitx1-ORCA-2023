function [X,Y,Z,V,surfHandle] = PlotTube_TzuChiao(d,varargin)
% 
...
defaults(end+1,:) = {'tubeColorMethod',{'pchip','spline','linear','nearest','next','previous','cubic','skip'},'nearest'};
...
if isempty(parameters.colormap)
    ...
    if ~strcmp(parameters.method,'skip')
        try
            % CS is probably where the color needs to change.
            cmap1 = interp1(CS, cmap(:,1), unique([linspace(0,CS(end),parameters.interpPts*n)]),parameters.tubeColorMethod); 
            cmap2 = interp1(CS, cmap(:,2), unique([linspace(0,CS(end),parameters.interpPts*n)]),parameters.tubeColorMethod); 
            cmap3 = interp1(CS, cmap(:,3), unique([linspace(0,CS(end),parameters.interpPts*n)]),parameters.tubeColorMethod); 
