function tubeFig = PlotPolymerTube_TzuChiao(xyz,varargin)

defaults = cell(0,3);
defaults(end+1,:) = {'fillmissing','boolean',true}; % not used
...
if sum(isnan(xyz(:,1)),1) == size(xyz,1)
   ...
   % plot
    ...
    if pars.showTube && ~pars.shortestPath
        PlotTube_TzuChiao(xyz,'r',pars.tubeRadius,'interpPts',pars.interpPts,'method',pars.method,...
                 'colormap',cmap,'lightingOn',false,'verbose',false); hold on;   
        set(gca,'color','w'); 
        shading flat;
        hold on;  
        alpha(pars.alpha);  
